...
export const DealOfTheDayCard = ({ data }) => {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
      {data?.slice(0, 8)?.map((item) => (
        <div
          key={item?.id}
          className="flex items-center justify-between gap-1.5 border rounded overflow-hidden group transition-all duration-300 hover:shadow-md hover:-translate-y-1 cursor-pointer"
        >
          ...
          <div className="w-[60%] p-3.5 space-y-3.5">
            ...
            <div className="bg-green-100 font-semibold rounded-md p-1.5 flex items-center justify-center gap-2">
              ...
              <span>You Can Save ${item?.price - item?.discountPrice}!</span>
            </div>
