...
export const FlashSaleCard = ({ data }) => {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-4 gap-5">
      {data?.slice(0, 8)?.map((item) => (
        <div
          key={item?.id}
          className="border rounded overflow-hidden group transition-all duration-300 hover:shadow-md hover:-translate-y-1 cursor-pointer"
        >
